...
namespace HotfixFramework.AssetBundle {
    public class ABManifestLoader : IDisposable {
        ...
        private UnityEngine.AssetBundle _ABReadManifest;
        ...
        public IEnumerator LoadMainifestFile() {
            using (WWW www = new WWW(_StrManifestPath)) {
                ...
                if (www.progress >= 1) {
                    ...
                    UnityEngine.AssetBundle abObj = www.assetBundle;
                    if (abObj != null) {
                        ...
                        Debug.Log(GetType() + "/LoadMainifestFile()/WWW下载出错，请检查！ _StrManifestPath=" + _StrManifestPath +
                                  "  错误信息： " + www.error);
                    }
                }
            }
