...
public class ProjectileCollisionBehaviour : MonoBehaviour {
    ...
    private void InitializeDefault() {
        ...
        if (!effectSettings.UseMoveVector) {
            forwardDirection = tRoot.position +
                               (tTarget.position + randomTargetOffsetXZVector - tRoot.position).normalized *
                               effectSettings.MoveDistance;
            GetTargetHit();
        }
        else
    ...
    private void Update() {
        ...
        if (Physics.Raycast(tRoot.position, direction, out raycastHit,
            distanceNextFrame + effectSettings.ColliderRadius, effectSettings.LayerMask)) {
            hit = raycastHit;
        ...
        var moveDistance = Vector3.MoveTowards(tRoot.position, endPoint,
            effectSettings.MoveSpeed * Time.deltaTime * accelerationY);
    ...
    private void GetTargetHit() {
        RaycastHit raycastHit;
        var ray = new Ray(tRoot.position,
            Vector3.Normalize(tTarget.position + randomTargetOffsetXZVector - tRoot.position));
    ...
    private void UpdateSmootRandomhPos() {
        ...
        if (IsDeviation) {
            ...
            coord2 = randomDirection3 *
                     Mathf.Sin(timeDegree + (randomDirection1 * Mathf.PI / 2) * time + Mathf.Sin(delta)) *
                     randomRadiusY * deviation;
        }
        else {
